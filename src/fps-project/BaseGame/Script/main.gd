...
@onready var light = preload("res://BaseGame/3D/light.tres") 
@onready var fusionx0 = preload("res://BaseGame/3D/fusionX0.tres")
@onready var fusionx1 = preload("res://BaseGame/3D/fusionX1.tres")
@onready var fusionx2 = preload("res://BaseGame/3D/fusionX2.tres")

@onready var hitRay = $fps/head/standCam/hitRay

@onready var CubeChanMaterial = preload("res://BaseGame/LifeStock/gclight.tres")
@onready var StickLightMaterial = preload("res://BaseGame/3D/xLight.tres")

@onready var fps = $fps # character body
@onready var LifeStock = $pleh/LifeMe #character body


func _follow_fps():
	if not Gl.GiveLife:
		if Gl.stateFirstAnim != 0:
			var direction = (fps.global_transform.origin - LifeStock.global_transform.origin).normalized()
			var target_rotation = atan2(direction.x, direction.z)  # Y ekseninde dönmesi için
			LifeStock.rotation.y = target_rotation
...
func _process(delta: float) -> void:
	...
	if fusionReadyArea:
		...
		if Input.is_action_just_pressed("e"):
			if Gl.fusion0 and Gl.fusion1 and Gl.fusion2:
				if not Gl.fusionReady:
					...
					Gl.fusionReady = true
					$fusionReadyx3/activeFusion/show.visible = false
					StickLightMaterial.emission_enabled = false

					# close light
					Gl.Index0LightOff = true
					$pleh/LifeMe/CubeChan/SpotLight3D.visible = false
					$pleh/LifeMe/CubeChan/SpotLight3D2.visible = false
					$pleh/LifeMe/CubeChan/SpotLight3D3.visible = false
					$pleh/LifeMe/CubeChan/SpotLight3D4.visible = false
		...
		if hitRay.is_colliding():
			var hit = hitRay.get_collider()
			if Input.is_action_just_pressed("e"):
				if hit.name == "settingsObject1Body":
					$fps/UI/hit.text = str(hit.name)
				elif hit.name == "settingsObject2Body":
					$fps/UI/hit.text = str(hit.name)
				elif hit.name == "settingsObject3Body":
					$fps/UI/hit.text = str(hit.name)
				elif hit.name == "settingsObject4Body":
					$fps/UI/hit.text = str(hit.name)
				elif hit.name == "settingsObject5Body":
					$fps/UI/hit.text = str(hit.name)
				elif hit.name == "settingsObject6Body":
					$fps/UI/hit.text = str(hit.name)
				elif hit.name == "settingsObject7Body":
					$fps/UI/hit.text = str(hit.name)

	$energyDC/rotate.rotate_y(0.5 * delta)
